%//%************************************************************************%
%//%*                              Ph.D                                    *%
%//%*                  Accleration to Displacement		     			   *%
%//%*                                                                      *%
%//%*       Author Name: Preetham Manjunatha                               *%
...
% Output_Data_ShakeTable1_Test1 -- 2 Hz and A = 0.1 inch
% data = data(1:10000,:);
...
% Sensor Constants
lvdtcons = 0.52725 /100;          % M/Volt
accbiasV = 2.5;                   % Vdc
accsensi = 1015/1000;   % V/mV/g
Lvdtmat = [];
...
%--------------------------------------------------------------------------
[~, ~, ~, filtered_acc_g] ...
          = func_accelodisp(time,Ts, Fs, Fcut,alpha, accval_g, Lvdtmat,...
                            lvdtcons, accbiasV, accsensi, filtertype...
                            ,filtermethod,firorder);
                        
[LVDTfilt, filtered_disp, filtered_vel, filtered_acc] ...
          = func_accelodisp(time,Ts, Fs, Fcut,alpha, accval, Lvdtmat,...
                            lvdtcons, accbiasV, accsensi, filtertype...
                            ,filtermethod,firorder);
